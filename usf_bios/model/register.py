...
from usf_bios.utils import HfConfigFactory, Processor, get_logger, is_unsloth_available, patch_getattr
...
class ModelLoader(BaseModelLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info_debug(f'model_kwargs: {model_kwargs}')
    ...
    def get_model(self, model_dir: str, config: PretrainedConfig, processor: Processor,
        ...
        if model is None:
            if self.return_dummy_model:
                ...
                logger.info_debug('Converting CausalLM to SequenceClassification for reranker task with num_labels=1')
                context = partial(patch_automodel_for_sequence_classification, **context_kwargs)
            else:
                context = partial(patch_automodel, **context_kwargs)
            with context():
        ...
        if model_info.task_type == 'embedding' and auto_model_cls.__name__ != 'AutoModel':
            from usf_bios.model.patcher import patch_output_normalizer
    ...
    def _add_new_special_tokens(self, model, tokenizer):
        ...
        if num_new_tokens > 0:
            logger.info_debug(f'Added {num_new_tokens} new special tokens.')
    ...
    def _deepspeed_set_z3_leaf_modules(self, model, z3_leaf_modules):
        ...
        if z3_leaf_modules:
            ...
            logger.info_debug(f'Setting z3_leaf_modules: {z3_leaf_modules}')
...
def get_model_processor(
    ...
    **kwargs,
    ...
    Args:
        model_id_or_path: The model identifier from a hub (HuggingFace/US Inc) or local path.
        ...
        # Hub parameters
        use_hf: Force using HuggingFace Hub (True) or US Inc (False). If None, it is controlled
