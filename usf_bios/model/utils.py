...
from usf_bios.utils import is_debug_mode, HfConfigFactory, Processor, deep_getattr, get_dist_setting, get_logger, is_mp, to_device
...
class AttnImpl:
    ...
    def update_attn_impl(config: PretrainedConfig,
        ...
        if attn_impl is None:
            return
        logger.info_debug(f'attn_impl: {attn_impl}')
...
def get_llm_model(model: "torch.nn.Module", model_meta=None, inner_backbone=True):
    ...
    if isinstance(model, (USFModel, PeftModel)):
        model = model.model
...
class InitModelStrategy:
    ...
    def init_parameters(model: "nn.Module", init_strategy: str) -> None:
        ...
        logger.info_debug(f'initialization strategy: {init_strategy}')
        ...
        for name, param in model.named_parameters():
            if InitModelStrategy.is_uninitialized(param):
                if is_debug_mode():
                    logger.info_debug(f'Initializing parameters: {name}.')
                init_func(param)
...
def _patch_conv3d():
    ...
    logger.info_debug('Conv3d patched successfully')


if strtobool(os.getenv('USF_PATCH_CONV3D', 'false')):
    _patch_conv3d()
...
def save_checkpoint(model: Optional[PreTrainedModel],
    ...
    if model is not None:
        if model.__class__.__name__ != 'SentenceTransformer':
            ...
            from usf_bios.utils import copy_files_by_pattern
