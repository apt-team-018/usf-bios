...
from usf_bios.utils import is_debug_mode, HfConfigFactory, get_logger, safe_snapshot_download
...
class ModelGroup:
    ...
    def __post_init__(self):
        assert not isinstance(self.template, (list, tuple))  # check usf-bios4.0
...
class ModelMeta:
    ...
    def __post_init__(self):
        ...
        assert not isinstance(self.loader, str)  # check usf-bios4.0
    ...
    if model_type is None:
        ...
        if len(model_types) > 1:
            raise ValueError(f'Failed to automatically match `model_type` for `{model_dir}`. '
                             ...
                             'https://usf-bios.readthedocs.io/en/latest/Instruction/Supported-models-and-datasets.html')
...
def get_model_info_meta(
    ...
    if model_type is None and model_info.model_type is not None:
        ...
        logger.info_debug(f'Setting model_type: {model_type}')
    ...
    if model_meta is None:  # not found
        if model_info.is_multimodal:
            ...
            logger.info_debug(f'Temporarily create model_meta: {model_meta}')
    if torch_dtype is None:
        torch_dtype = model_meta.torch_dtype or get_default_torch_dtype(model_info.torch_dtype)
        if is_debug_mode():
            logger.info_debug(f'Setting torch_dtype: {torch_dtype}')
    ...
    if task_type == 'reranker':
        ...
        logger.info_debug(f'Setting reranker task with num_labels={num_labels}')
    elif task_type == 'generative_reranker':
        # Generative reranker doesn't need num_labels as it uses CausalLM structure
        num_labels = None
        logger.info_debug('Setting generative_reranker task (no num_labels needed)')
