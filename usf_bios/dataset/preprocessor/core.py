...
from usf_bios.template import history_to_messages
from usf_bios.utils import get_logger, is_debug_mode, is_dist, is_master, safe_ddp_context
...
class RowPreprocessor:
    ...
    def __init__(self,
        ...
        # Statistics tracking for production mode
        self._total_samples = 0
        self._skipped_samples = 0
        self._skipped_max_length = 0
        self._skipped_other_errors = 0
        ...
        for row in rows:
            ...
            try:
                ...
                if isinstance(e, MaxLengthError):
                    self._skipped_max_length += 1
                    if ignore_max_length_error:
                        pass
                    elif is_debug_mode() and self._traceback_counter < self.traceback_limit:
                        import traceback
                        if is_debug_mode():
                            logger.info_debug(traceback.format_exc())
                        logger.warning('👆👆👆Sample skipped due to max_length error')
                        self._traceback_counter += 1
                else:
                    self._skipped_other_errors += 1
                    if is_debug_mode() and self._traceback_counter < self.traceback_limit:
                        import traceback
                        if is_debug_mode():
                            logger.info_debug(traceback.format_exc())
                        logger.warning('👆👆👆Sample skipped due to preprocessing error')
                        self._traceback_counter += 1
                row = []
            new_rows += row
    ...
    def get_preprocessing_stats(self) -> Dict[str, int]:
        """Get preprocessing statistics for summary reporting."""
        return {
            'total_samples': self._total_samples,
            'skipped_samples': self._skipped_samples,
            'skipped_max_length': self._skipped_max_length,
            'skipped_other_errors': self._skipped_other_errors,
            'remaining_samples': self._total_samples - self._skipped_samples,
        }
        ...
        *,
        ...
        if stats['skipped_samples'] > 0:
            logger.info_debug(
                ...
                f"(max_length={stats['skipped_max_length']}, other={stats['skipped_other_errors']}), "
                f"remaining={stats['remaining_samples']}")
        
        # Check for zero samples and graceful shutdown
        final_len = len(dataset_mapped) if isinstance(dataset_mapped, HfDataset) else None
        if final_len is not None and final_len == 0:
            error_msg = (
                f"No samples remaining after preprocessing! "
                f"Total processed: {stats['total_samples']}, "
                f"All skipped: {stats['skipped_samples']} "
                f"(max_length errors: {stats['skipped_max_length']}, other errors: {stats['skipped_other_errors']}). "
                f"Please check your dataset or increase --max_length."
            )
...
class ResponsePreprocessor(RowPreprocessor):
    """Dataset compatible with older versions of usf-bios"""
