...
from usf_bios.dataset import register_dataset_info
from usf_bios.utils import get_logger, json_parse_to_dict
...
class DataArguments:
    ...
    Args:
        ...
        cached_dataset (List[str]): Use cached datasets to avoid GPU time being occupied by tokenization during
            ...
            This is generated by the `usf export --to_cached_dataset true ...` command.
            Note: In `usf-bios>=3.11`, this only stores an extra 'length' field and filters out erroneous samples
        ...
        split_dataset_ratio (float): The ratio to split from the training set for validation if `val_dataset` is not
            provided. Defaults to 0.0. Note: The default was 0.01 in `usf-bios<3.6`.
        ...
        load_from_cache_file (bool): Whether to load the dataset from cache files. Recommended to set to `True` during
            ...
            Note: The default was `True` in `usf-bios<3.9`.
        ...
        model_name (Optional[List[str]]): For self-cognition tasks, replaces the `{{NAME}}` placeholder in the
            `usf_bios/self-cognition` dataset. Pass Chinese and English names.
            Example: `--model_name Xiao Huang 'Xiao Huang'`. Defaults to None.
        model_author (Optional[List[str]]): For self-cognition tasks, replaces the `{{AUTHOR}}` placeholder in the
            `usf_bios/self-cognition` dataset. Pass author's Chinese and English names.
            Example: `--model_author 'US Inc' 'US Inc'`. Defaults to None.
    ...
    model_name: Optional[List[str]] = field(default=None, metadata={'help': "e.g. ['Xiao Huang', 'Xiao Huang']"})
    model_author: Optional[List[str]] = field(default=None, metadata={'help': "e.g. ['US Inc', 'US Inc']"})
    ...
    def __post_init__(self):
        ...
        if len(self.val_dataset) > 0 or self.streaming and self.split_dataset_ratio > 0:
            ...
            logger.info_debug(f'Because {msg}, setting split_dataset_ratio: {self.split_dataset_ratio}')
