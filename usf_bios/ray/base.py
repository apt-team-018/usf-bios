...
from usf_bios.utils import find_free_port, find_node_ip
...
def try_init_ray():
    """Initialize Ray if --use_ray is set"""
...
class RayHelper:
            ...
            def wrapper(self, *args, **kwargs) -> T:
                ...
                if RayHelper.is_worker():
                    if not hasattr(self, 'group'):
                        # pass through env
                        self.group = os.environ['RAY_USF_GROUP'].split(',')
    ...
    def _create_workers(group: Union[str, List[str]], *args, **kwargs):
        ...
        exp_name = os.environ.get('RAY_USF_EXP_NAME')
        if not exp_name:
            exp_name = ''
        else:
            exp_name += '-'
        ...
        for _group in group:
            ...
            if _config['device'].upper() != 'CPU':
                ...
                for rank, (deploy_pg, gpu) in enumerate(zip(placement_groups, _config['ranks'])):
                    ...
                    env_vars['RAY_USF_GROUP'] = ','.join(local_groups)
                ...
                for deploy_pg, index in zip(placement_groups, list(range(world_size))):
                    ...
                    env_vars.update({
                        ...
                        VISIBLE_ENV_MAPPING[_config['device'].upper()]: '',
                        'RAY_USF_ARGS': get_args(),  # pass through env
                    })
                    env_vars['RAY_USF_GROUP'] = ','.join(local_groups)
