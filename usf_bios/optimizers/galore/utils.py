...
from usf_bios.utils import is_debug_mode, get_dist_setting, get_logger
from ..base import OptimizerCallback

if TYPE_CHECKING:
    from usf_bios.trainers import TrainingArguments
...
def _create_optimizer_and_scheduler(model: "nn.Module", args: 'TrainingArguments', config: GaLoreConfig, max_steps,
    ...
    for module_name, module in model.named_modules():
        ...
        if is_debug_mode():
            logger.info_debug(f'Enable GaLore for weights in module: {module_name}')
...
def get_optimizer(args: 'TrainingArguments', config: GaLoreConfig) -> Tuple[Any, Any]:
    ...
    if args.optim == 'adafactor':
        ...
        if config.quantize:
            ...
            logger.info_debug('If you encounter `absmax2` error, please downgrade your bitsandbytes to 0.40.0')
            from usf_bios.utils import get_dist_setting
