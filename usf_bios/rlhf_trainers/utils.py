...
from usf_bios.template import Messages
from usf_bios.tuners.lora import LoraConfig
from usf_bios.utils import (gc_collect, get_cu_seqlens_from_position_ids, get_logger, get_torch_device,
...
def aggressive_empty_cache(force_sync: bool = True, max_retries: int = 3) -> None:
    ...
    for attempt in range(max_retries):
        ...
        logger.info_debug(f'Memory cleanup attempt {attempt + 1}: Freed {reserved_freed / 1024**3:.2f} GB reserved, '
...
def memory_time_profiling_context(
    ...
    logger.info_debug(f'[{name}] Before: '
    ...
    logger.info_debug(f'[{name}] After: '
...
def patch_save_last_checkpoint():
    ...
    # patch to fix save last_checkpoint https://github.com/us-inc/usf-bios/pull/4969
...
def patch_vllm_moe_model_weight_loader(model):
    ...
    if getattr(model, '_usf_moe_weight_loader_patched', False):
        return
    ...
    original_model._usf_moe_weight_loader_patched = True
...
def get_chord_sft_dataloader(trainer,
    ...
    if not isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        if is_training:
            from usf_bios.utils import seed_worker
...
def compute_chord_loss(trainer, grpo_loss: torch.Tensor) -> torch.Tensor:
    ...
    """
    from usf_bios.trainers import per_token_loss_func
