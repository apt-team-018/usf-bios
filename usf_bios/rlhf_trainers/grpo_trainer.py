...
from usf_bios.plugins import orms, rm_plugins
from usf_bios.sequence_parallel import GatherLoss, sequence_parallel
from usf_bios.template import Template, TemplateInputs
from usf_bios.trainers import USFMixin, disable_gradient_checkpointing
from usf_bios.utils import (JsonlWriter, get_cu_seqlens_from_position_ids, get_logger, is_swanlab_available,
...
class GRPOTrainer(RolloutTrainerMixin, USFMixin, HFGRPOTrainer):
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        ...
        if self.overlong_filter and any(truncated_mask):
            if all(truncated_mask):
                logger.info_debug('All completions are overlong and truncated, '
        ...
        if reward_funcs:
            for i, reward_func in enumerate(reward_funcs):
                if reward_func in orms:
                    ...
                    raise ValueError(f'reward_function {reward_func} is not implemented in usf_bios.plugin')
        ...
        if reward_model is not None:
            ...
            for rm, rm_plugin, rm_template in zip(reward_model, reward_plugins, reward_templates):
                ...
                if rm_plugin not in rm_plugins:
                    raise ValueError(f'rm_plugin {rm_plugin} is not implemented in usf_bios.plugins')
