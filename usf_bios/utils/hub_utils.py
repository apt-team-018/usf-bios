...
def safe_snapshot_download(model_id_or_path: str,
    ...
    This function attempts to download a model from HuggingFace or HuggingFace Hub,
    ...
    Args:
        ...
        use_hf (Optional[bool], optional): Force using HuggingFace Hub (True) or US Inc (False).
    ...
    if check_local:
        ...
        if os.path.exists(model_suffix):
            ...
            logger.info_debug(f'Loading the model using local model_dir: {model_dir}')
            return model_dir
...
def git_clone_github(github_url: str,
    ...
    with safe_ddp_context('git_clone', use_barrier=True):
        ...
        if commit_hash is not None:
            ...
            subprocess_run(command)
    logger.info_debug(f'local_repo_path: {local_repo_path}')
    return local_repo_path
