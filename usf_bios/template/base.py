...
from usf_bios.utils import Processor, ProcessorMixin, get_env_args, get_logger, remove_response, retry_decorator, to_device
...
if TYPE_CHECKING:
    ...
    from usf_bios.infer_engine import InferRequest
...
class Template(ProcessorMixin):
    ...
    This class serves as the foundation for all template implementations in the USF BIOS framework.
    ...
    def __init__(
        ...
        *,
        ...
        # infer/deploy
        template_backend: Literal['usf_bios', 'jinja'] = 'usf_bios',
        # thinking
        ...
        """
        from usf_bios.loss_scale import LossScale, get_loss_scale
        from usf_bios.agent_template import agent_template_map
    ...
    def init_processor(self, processor: Processor) -> None:
        ...
        logger.info_debug(f'default_system: {repr(self.template_meta.default_system)}')
        logger.info_debug(f'max_length: {self.max_length}')
        logger.info_debug(f'response_prefix: {repr(self.response_prefix)}')
        logger.info_debug(f'agent_template: {self._agent_template}')
        if self.model_meta.is_multimodal:
            logger.info_debug(f'norm_bbox: {self.norm_bbox}')
    ...
    def _pre_tokenize_images(self, context_list: List[Context], loss_scale_list: List[float],
                             inputs: StdTemplateInputs) -> Tuple[List[Context], List[float]]:
        # https://github.com/us-inc/usf-bios/issues/3407
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            if context == '<image>' and inputs.is_multimodal and inputs.image_idx < len(inputs.images):
                ...
                inputs.image_idx += 1
                loss_scale = 0. if self.template_backend == 'usf_bios' else 1.
            else:
                c_list = [context]
            res += c_list
    ...
    def _usf_encode(self, inputs: StdTemplateInputs):
        template_meta = self.template_meta
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        self._usf_prepare_inputs(inputs)
        res_context_list, loss_scale_list, answer_len = (
            self._usf_encode(inputs) if template_backend == 'usf_bios' else self._jinja_encode(inputs))
        encoded = {}
        if self.is_encoder_decoder:
    ...
    def debug_logger(self, inputs):
        if not strtobool(os.getenv('USF_DEBUG', 'false')):
            return
    ...
    def data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        from usf_bios.dataset import RowPreprocessor
    ...
    def print_inputs(self, inputs: Dict[str, Any]) -> None:
        ...
        for key in keys_to_check:
            ...
            if val is not None:
                key_upper = key.upper()
                logger.info_debug(f'[{key_upper}_IDS] {val}')
                ...
                if isinstance(val, (list, tuple, torch.Tensor)):
                    ...
                    logger.info_debug(f'[{key_upper}] {val_str}')
        if inputs.get('loss_scale') is not None:
            val = inputs['loss_scale']
            logger.info_debug(f'[LOSS_SCALE] {val}')
