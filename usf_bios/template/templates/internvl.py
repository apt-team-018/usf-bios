...
from usf_bios.utils import get_env_args, is_deepspeed_enabled
...
register_template(
    ChatmlTemplateMeta(
        ...
        default_system='You are an AI assistant whose name is InternLM (InternLM).',
...
_internvl2_system = 'You are developed by Shanghai AI Laboratoryand SenseTime, an AI assistant named InternVL, designed to be helpful and harmless.'
register_template(
    ChatmlTemplateMeta(
...
register_template(
    ChatmlTemplateMeta(
        ...
        template_cls=Internvl2Template,
        default_system='You are InternVL, developed by Shanghai AI Laboratory, Tsinghua University, and other partners as a multimodal large language model.'))
...
class InternvlhfTemplate(Internvl2Template):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        from usf_bios.template.vision_utils import load_video_hf
        import numpy as np
        encoded = super(InternvlTemplate, self)._encode(inputs)
        input_ids = encoded['input_ids']
        labels = encoded['labels']
