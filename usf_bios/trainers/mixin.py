...
from usf_bios.dataloader import BatchSamplerShard, DataLoaderDispatcher, DataLoaderShard
from usf_bios.hub import get_hub
from usf_bios.loss import loss_map
from usf_bios.metrics import MeanMetric, compute_acc, metrics_map
from usf_bios.model import get_llm_model, get_lm_head_model, save_checkpoint
from usf_bios.model.patcher import gather_sequence_parallel_outputs, revert_padding_free, transformers_seq_cls_forward
from usf_bios.optimizers import OptimizerCallback, optimizers_map
from usf_bios.plugins import extra_tuners
from usf_bios.sequence_parallel import SequenceParallelDispatcher, SequenceParallelSampler, sequence_parallel
from usf_bios.template import Template, update_generation_config_eos_token
from usf_bios.tuners import USFModel
from usf_bios.utils import (HfConfigFactory, copy_files_by_pattern, deep_getattr, get_current_device, get_logger,
...
class USFMixin:
    ...
    def __init__(self,
        ...
        # Model version check based on source:
        # - Local path: skip (no remote check needed)
        # - HuggingFace (default or hf::): skip ModelScope check
        # - ModelScope (ms::): run ModelScope check
        model_path = getattr(args, 'model', '')
        is_local_path = os.path.isdir(model_path) if model_path else False
        use_hf = getattr(args, 'use_hf', True)  # Default is HuggingFace
        
        if args.check_model and hasattr(model, 'model_dir') and not is_local_path and not use_hf:
            # Only check ModelScope models (ms:: prefix, use_hf=False)
            with ms_logger_context(logging.CRITICAL), self._patch_timeout():
                config_info = self._collect_config_info()
                config_info.update({
                    'invoked_by': 'local_trainer',
                    'third_party': 'usf_bios',
            ...
            if isinstance(init_lora_weights, str):
                ...
                if 'lora-ga' in init_lora_weights:
                    try:
                        ...
                        """
                        logger.info_debug(error_message)
    ...
    def _save_model(self, output_dir: Optional[str] = None, state_dict=None):
        # model
        supported_classes = (USFModel, PreTrainedModel, PeftModel)
        ...
        if not isinstance(self.model, supported_classes) and self.model.__class__.__name__ not in supported_names:
            ...
            if isinstance(_unwrap_model, supported_classes):
                ...
                logger.info_debug('Trainer.model is not a `PreTrainedModel`, only saving its state dict.')
                if use_flash_ckpt:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        is_adapter = isinstance(self.model, (USFModel, PeftModel))
        # tokenizer
        if not is_adapter:
    ...
    def _rotate_flash_checkpoints(self, use_mtime=False, output_dir=None) -> None:
        ...
        for checkpoint in checkpoints_to_be_deleted:
            logger.info_debug(f'Deleting older checkpoint [{checkpoint}] '
        ...
        if is_deepspeed_zero3_enabled() and not hasattr(self.deepspeed, '_zero3_consolidated_16bit_state_dict_origin'):
            ...
            if 'exclude_frozen_parameters' in parameters:
                ...
                def _zero3_consolidated_16bit_state_dict(model, exclude_frozen_parameters=False):
                    ...
                    if isinstance(unwrapped, USFModel) and unwrapped.has_additional_modules:
    ...
    def _save_checkpoint(self, *args, **kwargs):
        ...
        logger.info_debug(f'Saving model checkpoint to {self.state.last_model_checkpoint}')
        return result
    ...
    def _save_flash_checkpoint(self, model, trial, metrics=None):
        ...
        if not success:
            logger.info_debug(f'Skip saving the checkpoint of step {self.state.global_step} '
    ...
    def _fix_gradient_checkpointing(self):
        ...
        if use_reentrant_ is None:
            if is_dist() and not self.is_deepspeed_enabled and not self.is_fsdp_enabled:
                ...
                use_reentrant_ = True
        logger.info_debug(f'use_reentrant: {use_reentrant_}')
    ...
    def train(self, *args, **kwargs):
        if self.model_meta.is_multimodal:
            ...
            logger.info_debug(f'Successfully registered post_encode hook: {[model.__class__.__name__ for model in models]}.')
