...
from usf_bios.utils import get_logger, patch_getattr
from .utils import USFAdapter, USFConfig, USFOutput

logger = get_logger()


@dataclass
class ReftConfig(USFConfig):
    """
...
class Reft(USFAdapter):
    ...
    def prepare_model(model: "nn.Module", config: ReftConfig, adapter_name: str):
        ...
        def save_callback(usf_model, model_dir, adapter_name):
            reft_model.save_intervention(save_directory=model_dir, include_model=False)

        def mark_trainable_callback(model):
            return

        def load_callback(usf_model, model_dir, adapter_name):
