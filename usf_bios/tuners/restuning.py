...
@dataclass
class ResTuningConfig(USFConfig):
    """
    ...
    def __post_init__(self):
        ...
        self.usf_type = USFTuners.RESTUNING
    ...
    def prepare_model(model: "nn.Module", config: ResTuningConfig, adapter_name: str) -> USFOutput:
        ...
        if config.root_modules:
            for module_key in module_keys:
                if re.fullmatch(config.root_modules, module_key):
                    ...
                    logger.info_debug(f'Matching root module [{module_key}] of type {type(root_module)}')
        ...
        for module_key in module_keys:
            if (isinstance(config.stem_modules, str) and re.fullmatch(config.stem_modules, module_key)) or \
                ...
                logger.info_debug(f'Matching stem module [{module_key}] of type {type(stem_module)}')
        ...
        for module_key in module_keys:
            if re.fullmatch(config.target_modules, module_key):
                ...
                logger.info_debug(f'Matching target module [{module_key}] of type {type(tgt_module)}')
