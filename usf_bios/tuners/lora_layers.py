...
from .utils import ActivationMixin, ModulesToSaveWrapper, USFAdapter
        ...
        for layer_name in self.adapter_layer_names:
            ...
            for key, layer in module_dict.items():
                if key in adapter_names and (not inference_mode):
                    ...
                    USFAdapter.save_memory(layer, key, self.module_key, True)
                else:
                    self.set_activation(key, False)
                    layer.requires_grad_(False)
                    USFAdapter.save_memory(layer, key, self.module_key, False, offload=offload)
    ...
    def save_memory(self, adapter_name, activate, offload=None):
        for layer_name in self.adapter_layer_names:
            ...
            for key, layer in module_dict.items():
                if key == adapter_name:
                    if activate:
                        USFAdapter.save_memory(layer, layer_name + '.' + key, self.module_key, True)
                    else:
                        USFAdapter.save_memory(layer, layer_name + '.' + key, self.module_key, False, offload=offload)
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        if target_base_layer.__class__.__name__ == 'NonDynamicallyQuantizableLinear':
            # Fix issue: https://github.com/us-inc/usf-bios/issues/342
            return None
