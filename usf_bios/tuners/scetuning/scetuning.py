...
from usf_bios.utils import find_sub_module, get_logger
from ..utils import ActivationMixin, USFAdapter, USFConfig, USFOutput
...
@dataclass
class SCETuningConfig(USFConfig):
    """
    ...
    def __post_init__(self):
        from usf_bios.tuners.mapping import USFTuners
        self.usf_type = USFTuners.SCETUNING
    ...
    def prepare_model(model: "nn.Module", config: SCETuningConfig, adapter_name: str) -> USFOutput:
        ...
        if config.hint_modules:
            if isinstance(config.hint_modules, list):
                for module_key in config.hint_modules:
                    ...
                    logger.info_debug(f'Matching hint module [{module_key}] of type {type(h_module)}')
        ...
        if isinstance(config.target_modules, list):
            for module_key in config.target_modules:
                ...
                logger.info_debug(f'Matching target module [{module_key}] of type {type(t_module)}')
        ...
        if len(hint_module_ins_list) > 0 and not len(hint_module_ins_list) == len(target_module_ins_list):
            logger.info_debug("Target modules' length should be equal with hint modules.")
