...
from usf_bios.utils.constants import DEFAULT_ADAPTER, USF_TYPE_KEY
from .mapping import USFTuners
from .peft import PeftConfig, PeftModel, get_peft_model
from .utils import USFConfig, USFOutput

logger = get_logger()


class USFModel(nn.Module):
    """The USF wrapper model.

    Args:
        model (`Union[nn.Module, 'USFModel']`) A module to be tuned by USF.
        config (`Union[USFConfig, Dict[str, USFConfig]]`) A config or a dict of {adapter_name: USFConfig}.
    ...
    def __init__(self,
                 model: Union[nn.Module, 'USFModel'],
                 config: Union[USFConfig, Dict[str, USFConfig]],
        ...
        new_adapters = []
        if isinstance(config, USFConfig):
            ...
            assert (all(isinstance(c, USFConfig) for c in config.values()))
    ...
    def _deactivate_all_parts(self):
        ...
        for adapter in self.active_adapters:
            ...
            if output.config.usf_type == USFTuners.PART:
    ...
    def load_state_dict(self, state_dict, strict=True, adapter_name: str = None):
        if adapter_name is not None:
            output: USFOutput = self.adapters[adapter_name]
    ...
    def load_state_file(path, device: Optional[str] = None):
        ...
        if os.path.exists(os.path.join(path, SAFETENSORS_WEIGHTS_NAME)):
            ...
            raise NotImplementedError(f'USF BIOS does not support {WEIGHTS_NAME} to prevent malicious code. '
    ...
    def from_pretrained(cls,
        ...
        Args:
            model (`Union[torch.nn.Module, 'USFModel']`): The model to be tuned,
                if the model is already a `USFModel` it will be un-wrapped and re-wrapped..
        ...
        for _name in adapter_name if isinstance(adapter_name,
            ...
            if USF_TYPE_KEY not in json_object:
                ...
                adapters[key] = USFConfig.from_pretrained(sub_folder)

        self = USFModel(model, adapters, extra_state_keys, inference_mode, **kwargs)
        for _name in adapter_name if isinstance(adapter_name,
                                                list) else [adapter_name] \
                if isinstance(adapter_name, str) else adapter_name.keys():
            _adapter = _name if not isinstance(adapter_name, dict) else adapter_name[_name]
            output: USFOutput = self.adapters[_adapter]
            sub_folder = os.path.join(model_dir, _name)
    ...
    def _prepare_model(
        ...
        assert (hasattr(config, USF_TYPE_KEY))
        from .mapping import USF_MAPPING

        adapter_cls = USF_MAPPING[config.usf_type][1]
    ...
    def create_or_update_model_card(self, output_dir: str):
        ...
        from usf_bios.version import __version__
        if framework_block_heading in lines:
            lines.insert(lines.index(framework_block_heading) + 2, f'- USF BIOS {__version__}\n')
        else:
            lines.append(f'{framework_block_heading}\n\n- USF BIOS {__version__}\n')
    ...
    def add_weighted_adapter(
        ...
        """
        from usf_bios.tuners.lora import LoraModel
    ...
    def save_pretrained(self,
        ...
        for adapter_name, output in self.adapters.items():
            ...
            save_to_peft = peft_format and output.config.usf_type == USFTuners.LORA
    ...
    def activate_adapter(self, adapter_name: str):
        ...
        from .mapping import USF_MAPPING
        USF_MAPPING[self.adapters[adapter_name].config.usf_type][1]\
...
class USF:
    """The Wrapper to use both Peft and USF tuners."""

    @staticmethod
    def prepare_model(model: Union[nn.Module, USFModel], config: Union[USFConfig, PeftConfig,
                                                                         Dict[str, USFConfig]], **kwargs):
        """Prepare a model by the input config.

        Args:
            model(`Union[nn.Module, 'USFModel']`): The model to be tuned.
            config(`Union[USFConfig, PeftConfig, Dict[str, USFConfig]]`): The config or config dict, can be either
                USFConfigs or PeftConfigs
            **kwargs:
                Extra kwargs needed by USFModel or PeftModel.
        Returns:
            The model wrapped by USFModel or PeftModel.
        """

        if isinstance(config, (USFConfig, dict)):
            return USFModel(model, config, **kwargs)
        else:
            return get_peft_model(model, config, **kwargs)

    @staticmethod
    def merge_and_unload(model: Union[PeftModel, USFModel], **kwargs):
        """Merge tuners into the base model and unload them.

        Args:
            model(`Union[PeftModel, USFModel]`): The model instance with tuners
            kwargs:
                adapter_name(`Union[str, List[str]]`): The adapter_name to unload, only supported in usf tuners.

        """
        from peft import PeftModel as _PeftModel
        if isinstance(model, _PeftModel):
            model.merge_and_unload()
        elif isinstance(model, USFModel):
            from usf_bios.tuners import LoRAConfig, LoRA
    ...
    @staticmethod
    @contextmanager
    def grpo_context(model: Union[USFModel, torch.nn.Module], processor):
        # Save the model and temporarily modify model.model_dir.
        if not isinstance(model, USFModel):
            yield
            return
        else:
            assert len(model.adapters) == 1
            adapter = list(model.adapters.values())[0]
            if adapter.config.usf_type == USFTuners.LLAMAPRO:
    ...
    @staticmethod
    def merge(model: Union[PeftModel, USFModel], **kwargs):
    ...
    @staticmethod
    def unmerge(model: Union[PeftModel, USFModel], **kwargs):
    ...
    def save_to_peft_format(ckpt_dir: str, output_dir: str) -> None:
        """Save usf format to peft format

        Args:
            ckpt_dir(`str`): Original usf output dir
        ...
        if os.path.exists(os.path.join(ckpt_dir, USFModel.EXTRA_STATE_DIR)):
        ...
        def has_custom_content(_json):
            if _json.get('usf_type', _json.get('peft_type')) != USFTuners.LORA:
                logger.warn('Only LoRA can be converted to peft format')
                return True

            from usf_bios.tuners import LoRAConfig
        ...
        for adapter in adapter_names:
            ...
            state_dict = USFModel.load_state_file(os.path.join(output_dir, adapter))
            new_state_dict = {}
            for key, value in state_dict.items():
                if not key.startswith('base_model.model.'):
                    key = 'base_model.model.' + key
            ...
            USFModel._save_state_dict(state_dict, os.path.join(output_dir, adapter), safe_serialization)
            from usf_bios.tuners import LoRAConfig
        ...
        if 'default' in adapter_names:
            ...
            state_dict = USFModel.load_state_file(os.path.join(output_dir, 'default'))
            safe_serialization = os.path.isfile(os.path.join(output_dir, 'default', SAFETENSORS_WEIGHTS_NAME))
            USFModel._save_state_dict(state_dict, output_dir, safe_serialization)
            shutil.rmtree(os.path.join(output_dir, 'default'))

    @staticmethod
    def from_pretrained(model: Union[nn.Module, USFModel, PeftModel],
                        model_id: str = None,
                        adapter_name: Union[str, List[str], Dict[str, str]] = None,
                        revision: str = None,
                        **kwargs):
        """Prepare a model by a model_id in the HuggingFace Hub or a local dir.
        ...
        Args:
            ...
            **kwargs:
                Extra kwargs needed by ``USFModel.from_pretrained`` or ``PeftModel.from_pretrained``.
        Returns:
            The model wrapped by USFModel or PeftModel.
        """
        ...
        if os.path.exists(os.path.join(model_id, CONFIG_NAME)):
            with open(os.path.join(model_id, CONFIG_NAME), 'r', encoding='utf-8') as f:
                _json = json.load(f)
            is_peft_model = USF_TYPE_KEY not in _json
        ...
        if os.path.exists(os.path.join(model_id, _name, CONFIG_NAME)):
            with open(os.path.join(model_id, _name, CONFIG_NAME), 'r', encoding='utf-8') as f:
                _json = json.load(f)
            is_peft_model = USF_TYPE_KEY not in _json and 'extra_state_keys' not in _json
        if is_peft_model:
            ...
            return USFModel.from_pretrained(model, model_id, revision=revision, adapter_name=adapter_name, **kwargs)
