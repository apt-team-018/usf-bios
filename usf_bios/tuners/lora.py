...
from .utils import USFAdapter, USFConfig, USFOutput, set_adapter

logger = get_logger()


@dataclass
class LoRAConfig(LoraConfig, USFConfig):
    """
    ...
    def to_peft_config(self) -> 'LoraConfig':
        ...
        _dict['peft_type'] = _dict['usf_type']
        _dict.pop('usf_type', None)
    ...
    def unpatch_lora(model, config: "LoRAConfig", adapter_name: str):
        ...
        if not config.use_merged_linear:
            if version.parse(peft.__version__) < version.parse('0.6.3'):
                logger.info_debug('All adapters will be merged.')
