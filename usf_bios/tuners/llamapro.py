...
from usf_bios.utils import HfConfigFactory, get_logger
...
@dataclass
class LLaMAProConfig(USFConfig):
    """
    ...
    def __post_init__(self):
        ...
        self.usf_type = USFTuners.LLAMAPRO
...
class LLaMAPro(USFAdapter):
    ...
    def prepare_model(model: "nn.Module", config: LLaMAProConfig, adapter_name: str) -> USFOutput:
        ...
        try:
            ...
            if model_type is None:
                language_model_name = USFAdapter.get_model_key_mapping(config.model_type, config).language_model
    ...
    def get_model_key_mapping(cls, model_type, config) -> ModelKeys:
        ...
        model_key_mapping = USFAdapter.get_model_key_mapping(model_type, config)
