...
from usf_bios.model import MODEL_ARCH_MAPPING, ModelKeys
from usf_bios.utils import gc_collect, get_logger
from usf_bios.utils.constants import BIN_EXTENSIONS
...
class USFConfig:
    ...
    usf_type: str = field(default=None)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        assert loaded_attributes.get('usf_type', '') in USF_MAPPING
        config = USF_MAPPING[loaded_attributes['usf_type']][0](**kwargs)
...
class USFOutput:
    ...
    Args:
        ...
        config (`USFConfig`): The usf config instance.
...
class USFAdapter:
    ...
    def save_memory(module: "torch.nn.Module", adapter_name: str, module_key: str, activate: bool, offload: str = None):
        if not isinstance(module, torch.nn.Module):
            return
        if activate:
            USFAdapter.load(module, adapter_name, module_key)
        else:
    ...
    def offload(module: "torch.nn.Module", adapter_name, module_key, offload: str):
        ...
        if offload == 'cpu':
            ...
            if str(device) != 'meta':
                if USFAdapter.offload_helper is None:
                    USFAdapter.offload_helper = OffloadHelper()
                USFAdapter.offload_helper.offload_disk(module, adapter_name=adapter_name, module_key=module_key)
                module.to('meta')
        else:
            raise NotImplementedError
        gc_collect()
    ...
    def load(module: "torch.nn.Module", adapter_name, module_key):
        ...
        if str(device) == 'cpu':
            ...
            USFAdapter.offload_helper.load_disk(module, adapter_name=adapter_name, module_key=module_key)
            module.to(module.origin_device)
            delattr(module, 'origin_device')
...
class ModulesToSaveWrapper(ActivationMixin, _ModulesToSaveWrapper):
    ...
    def __init__(self, *args, module_key, **kwargs):
        ...
        USFAdapter.save_memory(self.original_module, 'original_module', self.module_key, False, offload='cpu')
    ...
    def set_adapter(self, adapter_name: str, offload: str = None):
        ...
        USFAdapter.save_memory(self.modules_to_save[adapter_name], adapter_name, self.module_key, True)
        USFAdapter.save_memory(self.original_module, 'original_module', self.module_key, False, offload=offload)
    ...
    def deactivate_adapter(self, adapter_name: str, offload: str = None):
        ...
        USFAdapter.save_memory(
            self.modules_to_save[adapter_name], adapter_name, self.module_key, False, offload=offload)
        if not self.get_activated_adapters():
            USFAdapter.save_memory(self.original_module, 'original_module', self.module_key, True)

    def enable_adapters(self, enabled: bool):
        super().enable_adapters(enabled)
        if not enabled:
            USFAdapter.save_memory(self.original_module, 'original_module', self.module_key, False, offload='meta')
        else:
            USFAdapter.save_memory(self.original_module, 'original_module', self.module_key, True)
...
def usf_to_peft_format(ckpt_dir: str, output_dir: str) -> str:
    if 'default' in os.listdir(ckpt_dir):  # usf_backend
        from usf_bios import USF
        USF.save_to_peft_format(ckpt_dir, output_dir)
        ckpt_dir = output_dir
        logger.info_debug(f'Converting the usf format checkpoint to peft format, and saving it to: `{output_dir}`')
    else:
        logger.info_debug('The format of the checkpoint is already in peft format.')
    return ckpt_dir
