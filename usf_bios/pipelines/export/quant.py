...
class QuantEngine(ProcessorMixin):
    ...
    def quantize(self):
        ...
        if is_debug_mode():
            logger.info_debug(f'model: {self.model}')
        if is_debug_mode():
            logger.info_debug(f'model_parameter_info: {get_model_parameter_info(self.model)}')
        save_checkpoint(
            None,
            self.processor,
            args.output_dir,
        ...
        logger.info_debug(f'Successfully quantized the model and saved in {args.output_dir}.')
    ...
    def _get_quant_dataset(self, *args, **kwargs):
        ...
        logger.info_debug(f'quant_dataset: {dataset}')
        ...
        logger.info_debug(f'Split into {n_split} blocks')
        res = []
    ...
    def awq_model_quantize(self) -> None:
        ...
        args = self.args
        logger.info_debug(f'Quantization dataset: {args.dataset}')
        ...
        if is_debug_mode():
            logger.info_debug(f'quant_config: {quant_config}')
        logger.info_debug('Start quantizing the model...')
    ...
    def gptq_model_quantize(self, v2: bool = False):
        ...
        logger.info_debug(f'block_name_to_quantize: {block_name_to_quantize}')
        logger.info_debug(f'modules_in_block_to_quantize: {modules_in_block_to_quantize}')
